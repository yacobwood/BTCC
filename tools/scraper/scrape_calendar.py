...
def scrape_calendar(year: int) -> list[dict] | None:
    ...
    html = fetch_via_scrapfly(CALENDAR_URL, render_js=True, label="calendar")
    if html is None:
        print("ERROR: could not fetch calendar (Scrapfly fetch failed)", file=sys.stderr)
        return None
...
def main():
    ...
    for s in schedule:
        ...
        if not slug:
            continue
        entries = _ft.scrape_circuit_timetable(slug, referer=CALENDAR_URL)
        if entries:
