...
//
// TTB deployment for Races (laps of boost available) is a sliding scale by
// position - P1 gets the fewest laps, P8-or-lower gets the most, so the
// system narrows the field rather than compounding an advantage. The
// position source differs by race:
//   Race 1 -> Championship Order (points total before this round)
//   Race 2 -> Race 1 finishing order (same meeting)
//   Race 3 -> Race 2 finishing order (same meeting)
// Circuit split (reg 1.11.1.b): "B circuits" (Brands Hatch Indy, Knockhill,
...
//
// Round 1's Race 1 has no prior Championship Order to rank by, and the regs
// don't define a fallback for that gap (every other "no order yet" case in
// these regs is handed to Co-ordinator discretion, not a computed rule). Per
// user confirmation (2026-08-19, observed rather than a quoted regulation -
// see memory/project_ttb_boost_allocation.md): every driver gets the maximum
// TTB tier (the same P8+ row already used to cap the scale) at the season
// opener. ttbPositionMap applies that fallback for Race 1 whenever
