...
**Hardcoded driver/team images** - `data/driverImages/`, `data/carImages/`, `data/numberImages/` and `data/backgroundImages/` hold official team/driver graphics (headshot, side-on car cutout, branded number graphic, 1920x600 card background respectively), named `<car number>.png` (team backgrounds instead use `<team-slug>.png`/`.jpg`, plus one `driver-<number>.png` override for a driver whose own card differs from their team's). Referenced by `raw.githubusercontent.com` URL from `data/drivers.json`: driver-level `imageUrl`/`carImageUrl`/`numberImageUrl`/`cardBgUrl`, team-level `cardBgUrl`/`carImageUrl` (`attachTeamDisplayFields()` in `src/api/parsers.js` prefers a driver's own `cardBgUrl`/`carImageUrl` over their team's, same precedence for both fields). `numberImageUrl` replaced the plain styled-text car number in `DriversScreen`/`DriverDetailScreen`/`TeamDetailScreen` (falls back to the old text rendering when absent). No scraper writes any of these four fields - swapping an image means committing a new file under the same name (or repointing the URL in `drivers.json` if the name changes), no code change or app release needed. Two current drivers still have a gap (Senna Proctor has none of the four; Nick Halstead has no headshot) pending a real file to drop in.
