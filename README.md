...
`carStrip`'s `aspectRatio` is `1536 / 1024` - deliberately matching the canvas every `data/carImages/` file is generated to (see `carThumbUrl`'s own comment), not an arbitrary banner shape. This mattered more than it looks: an initial pass used `aspectRatio: 2.6` (wider than the image's real 1.5:1), and no matter how large `carStripImg`'s own width/height percentages were set, `resizeMode="contain"` stayed height-bound against that mismatched box shape, capping the rendered car at roughly half the strip's width - confirmed by checking the actual source file (`PIL.Image.open(...).size` -> `(1536, 1024)`) rather than guessing. Once the box's own aspect ratio matched the canvas, `carStripImg`'s `94%/94%` (widened from `85%/85%`, by request - "almost full width") could actually take effect, since both axes now bind together instead of one always capping the other first.
