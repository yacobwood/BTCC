...
| `explainer_articles.json` | "Explained" regulation-explainer articles, `{posts: [{id, title, content, status: 'staged'\|'published', scheduledDate, pubDate, ...}]}` - `status` is admin-flipped via the admin page's EXPLAINED tab, not date-automated; see the ExplainerListScreen entry in [§6](#6-screens-reference) |
...
| `explainer_alerts` | New "Explained" regulation-explainer article published - see the Explained section entry below and [§6](#6-screens-reference) |
...
| `results_teaser` | Session results posted - spoiler-safe copy, deliberately excluded from `RESULT_LEAF_KEYS` so `spoilerFree` never suppresses it (added 2026-08-25, growth/re-engagement report) |
...
Session results notifications are a separate mechanism entirely - `.github/scripts/session_watcher.py` (Python, connected to TSL's live-timing SignalR feed, not this Cloud Function) sends those on `sessioncomplete` events, since it needs to react the moment a session actually finishes rather than poll once a minute. **Still not reachable in production as of 2026-09-02**, though the reason changed: `race-day-start.yml`'s dispatch bug (root-caused and fixed, see §20) turned out to be masking a second, deeper problem - the script's own `load_sessions()` no longer matches `data/schedule.json`'s current shape and raises `KeyError` immediately on any real dispatch, before ever reaching the notification code. Confirmed this fails safely (no duplicate/broken notifications, just a visible failed Actions run) rather than causing live harm. See [§20](#20-python-scrapers).
...
**session_watcher.py** (`.github/scripts/`) - Connects to TSL's SignalR live-timing feed for a race day and reacts to `sessioncomplete` events: fires a "Starting in 15 mins" pre-session alert, and on each session's completion waits 3 minutes for the PDF, scrapes+commits, then sends that session's results notification. **Root cause found 2026-09-02** for why it never actually ran (previously an open mystery, noted below as "hasn't run since May"): `race-day-start.yml`'s own `start_race_day.py` was reading `data/schedule.json`'s `saturday_date`/`sunday_date` fields to decide whether to auto-dispatch it - those fields never existed there (confirmed 100% failure on rounds 6/7's real race days via run logs). Fixed (`15d120bc`) to read `data/calendar.json`'s `startDate`/`endDate` instead, same source `is_race_weekend.py` already used correctly. **Still not actually reachable in production though** - this script's own `load_sessions()` expects a `schedule.json` shape that no longer exists (`rnd["tsl"]`, `rnd["venue"]`, `rnd["sessions"]` as a dict keyed by day with `suffix`/`start_utc`/`notify_pre`/`is_q1`/`pre_label` per session); the real file only has `round` + a flat `sessions` list of `{name, day, time}`, so `rnd["tsl"]` raises `KeyError` immediately - confirmed live. This happens before any notification thread starts, so the practical effect of the dispatch fix alone is a clean, visible failed run in Actions each race weekend, not a live crash or duplicate notifications (its `PRE_TOPICS` are identical to `sendSessionNotifications`' own `SESSION_TOPICS` - `pre_fp`, `pre_race1`, etc. - so if it ever did run, it would double-send every pre-session alert on top of the one below that already works). Every pre-session notification a user actually receives today comes from `sendSessionNotifications` instead (see [§12](#12-firebase-cloud-functions)); results land via `scrape-results.yml`'s own polling + `notifyResultsUpdate`'s spoiler-safe generic teaser (also below) - the one thing genuinely still missing is a spoiler-specific per-session result push (e.g. "Race 1: Sutton wins"), which only this script ever sent. Deliberately not rushed into a full fix 3 days before round 8 (Croft) - restoring it needs a `load_sessions()` rewrite against the current data shapes, ideally tested against a live TSL feed rather than shipped untested right before a real race weekend. This file's implementation is otherwise left in place, correct in its own logic, dormant pending that rewrite.
