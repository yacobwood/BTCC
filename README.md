...
| `results_teaser` | Session results posted - spoiler-safe copy, deliberately excluded from `RESULT_LEAF_KEYS` so `spoilerFree` never suppresses it (added 2026-08-25, growth/re-engagement report). Sent by `functions/scraperAdmin.js`'s `notifyResultsUpdate`, called by `scrape-results.yml` on every scrape tick (every 2 min on a raceday) - had **no dedup at all** until 2026-09-05, unconditionally firing on every call. `scrape_tsl.py` re-stamps `standings.json`'s `updated` field on nearly every raceday run regardless of real content change (`SESSION_FILTER` deliberately keeps grid-bearing sessions open all day "to catch grid amendments"), which also makes the workflow's own `changes_detected` gate fire almost every tick - confirmed live, users getting repeat "A fresh result just dropped" pushes during the Croft (round 8) weekend with nothing new to show. Fixed with `functions/resultsHash.js`'s `computeResultsHash()` - hashes `results{year}.json` + `standings.json` (excluding `updated`) and only sends when it differs from `state/results_teaser`'s stored hash in Firestore. |
