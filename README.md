...
The header (added 2026-08-21) also shows the driver's own car as a bottom-left corner badge overlaid on the photo, same underlying reason as DriversScreen's tile - a team can field more than one livery, so it has to be the driver's own `carImageUrl`, not a shared team image - but not the same layout: the tile moved its car into its own non-overlapping strip once sized up (see that entry above for why), while this screen's much larger header keeps the overlay, since there's enough room here for the car to sit convincingly at the driver's feet rather than visibly clipping through them. Sized as a genuine showcase on this one-driver page (68%/45% of the header, ~1.5 aspect matching the car cutout's own) rather than matched to the tile's smaller footprint - went through two smaller passes (22%/14%, then 48%/32%) that each still read as too small on-device before landing here. Uses the same pre-generated `-thumb` URL (`carThumbUrl()`, a third identical copy of DriversScreen's/TeamDetailScreen's helper) as everywhere else this renders - this screen never actually hit the decode-memory cap that motivated adding it (only one driver's car is ever on screen here), but there's no reason to decode 15x more bitmap than a corner badge needs just because the count happens to be small in this context.
