...
The header (added 2026-08-21) also shows the driver's own car as a bottom-left corner badge, same treatment and same reason as DriversScreen's tile - a team can field more than one livery, so it has to be the driver's own `carImageUrl`, not a shared team image. Sized smaller as a % of the container than the tile's equivalent badge (22%/14% vs. 46%/30%) since this header is full screen width rather than a half-width tile - the same percentage here would roughly double the badge's absolute size just because the container got bigger, not because the badge should be any more prominent. Uses the same pre-generated `-thumb` URL (`carThumbUrl()`, a third identical copy of DriversScreen's/TeamDetailScreen's helper) as everywhere else this renders - this screen never actually hit the decode-memory cap that motivated adding it (only one driver's car is ever on screen here), but there's no reason to decode 15x more bitmap than a corner badge needs just because the count happens to be small in this context.
