...
The header (added 2026-08-21) also shows the driver's own car - same underlying reason as DriversScreen's tile used to have - a team can field more than one livery, so it has to be the driver's own `carImageUrl`, not a shared team image. Now that the tile has dropped its car entirely (see above), this header is the *only* place it appears - so it's been sized to make the most of being the one dedicated showcase, rather than just matching the tile's old proportions. `CAR_SIDE_TOP` tightened from roughly halfway down the header (`HEADER_W * 0.5`) to `HEADER_W * 0.38`, matching where `headerNumberImg`'s own 36% height ends, so the strip starts immediately under the number instead of leaving a gap above it, and (via `CAR_SIDE_H`, unchanged in formula) still runs the rest of the way down to the header's bottom edge - full height between the number and the bottom, by request. `CAR_SIDE_W`, the strip's final post-rotation width, widened from `HEADER_W * 0.24` to `0.34` for a visibly bigger car to match. Otherwise the same arrangement as before: `headerNumberImg`/`headerNumber` stay top-right, `headerPhoto` narrowed 100% -> 60% width and left-aligned (`headerBg`'s `alignItems: 'flex-start'`), and the car runs down the right edge underneath the number in a vertical strip rotated -90 degrees anti-clockwise (`headerCarSide`/`headerCarSideImg`, sized off `HEADER_W`-derived pixel constants for the percentage-vs-rotation reason detailed in the tile's entry above, back when the tile had the identical problem to solve). Went through several earlier passes first: a bottom-left overlay (fine when the car was smaller, since there was enough headroom for it to sit at the driver's feet without clipping through them - see below for the sizes tried: 22%/14%, then 48%/32%, then 68%/45%), which stopped working once the car grew large enough to visibly clip the photo; then an upright bottom-right corner badge (`headerCarImg`, 50%/33%) that fixed the clipping but capped how large the car could get; then a rotated strip at the tile's own then-current proportions - widened and tightened further once the tile's car was removed and this became the app's one remaining place to show it. Uses the same pre-generated `-thumb` URL (`carThumbUrl()`) as `TeamDetailScreen`'s per-driver cards - this screen never actually hit the decode-memory cap that originally motivated the thumbnail (only one driver's car is ever on screen here), but there's no reason to decode 15x more bitmap than a strip this size needs just because the count happens to be small in this context.
