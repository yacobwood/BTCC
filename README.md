...
A different, unrelated failure mode reported live 2026-09-08 on `TrackDetailScreen`: not the FAB overlapping content, but a large empty gap appearing *above* it. `CHAT_FAB_CLEARANCE` only reserves space once a user has scrolled to the true end of a screen's content - it has no effect before that, and can't, since the FAB's own position is fixed relative to the tab bar regardless of how much content precedes it. `TrackDetailScreen`'s FlatList renders the whole page (hero, stats, records, schedule, weather, layout, then the circuit guide) as one list, and RN's `ScrollView` base style always stretches the scroll viewport to fill the screen even when the rendered content is shorter - so a circuit whose guide happens to end on a short final corner description (Silverstone's one-sentence Woodcote/T5, the last corner of its last sector) left a large stretch of bare background between the text and the FAB. Centering the content vertically (`flexGrow: 1` + `justifyContent: 'center'`) was considered and rejected - `TrackDetailScreen`'s hero/title is a sticky header (`stickyHeaderIndices`), and centering the whole list would pull that away from the top of the screen on exactly the short-content cases this was meant to fix. Padding doesn't help either, for a more basic reason: the gap is already empty space, and padding is also empty space - adding more of it either changes nothing visible or makes the scrollable void larger, never smaller. Fixed instead with a small closing element (`guideFooter` item type, a divider + "END OF CIRCUIT GUIDE" caption) appended after the last sector - genuinely visible content in the space, rather than more invisible padding or a centering trick that would have broken the sticky header elsewhere.
