...
**session_watcher.py** (`.github/scripts/`) - Connects to TSL's SignalR live-timing feed for a race day and reacts to `sessioncomplete` events: fires a "Starting in 15 mins" pre-session alert, and on each session's completion waits 3 minutes for the PDF, scrapes+commits, then sends that session's results notification. When the completed session is **Race 2** it also spawns a background thread (`watch_for_race3_grid`) that polls (every 2 min, up to 20 min) for Race 3's own grid to appear and sends a separate `pre_race3_grid` notification once it does - deliberately does *not* re-run the scraper itself (that's `scrape-results.yml`'s own 2-minute polling, already re-fetching grid-bearing sessions per `is_race_weekend.py` above; running it twice would risk two processes racing to commit the same file), it only re-pulls and re-reads the committed results file. This poll-rather-than-assume approach exists because per reg 3.4.1.b the reverse-grid reversal count is "picked at random... as soon as practical after the finish of that Race" - a separately-timed event from Race 2's finishing order going final, so a notification fired the instant Race 2's own result lands would risk announcing a grid that isn't actually published yet. Notification payload is `{type: 'results', round, year, race: '3'}`, reusing `notifNavigation.js`'s existing race-index deep-link handling unchanged (see [§11](#11-notifications-system)). Pure grid-lookup logic (`find_race3`, `race3_grid_ready`, `race3_pole_sitter`) is unit tested in `test_session_watcher.py`.
