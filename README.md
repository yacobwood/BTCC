...
**Share crash on Academy articles fixed (2026-09-06):** `onShare` called `.replace()`/`.split()` on `article.link` unconditionally, but Academy/explainer articles always have `link: null` (`mapExplainerPosts` in `api/client.js` - they're in-app-only content with no matching btcc.net page), so sharing one crashed every time. Falls back to the generic app link (same pattern as `utils/appShare.js`) when `article.link` is null.

**WebView font-boosting fix (2026-09-11):** confirmed live on a Pixel 10a with the system font size turned up - the injected CSS had no `text-size-adjust: 100%` rule, so Android WebView's own font-boosting heuristic (separate from, and additional to, the OS accessibility text-size setting) rescaled individual paragraphs unpredictably on this narrow single-column layout, producing a paragraph that rendered hugely oversized and superimposed directly on top of the next one, both illegible. `buildHtml()` now sets `html { -webkit-text-size-adjust:100%; text-size-adjust:100%; }`, which fixes the article body's own font size regardless of the OS setting - a deliberate trade-off of accessibility scaling for a legible, stable layout, since the alternative was actively broken.
...
**Saturday/Sunday weather view inconsistency, fixed for real 2026-09-09 (2026-09-07's own fix reversed the same day it shipped):** a past-day filter (`weather.daily.filter(day => new Date(day.date) >= today)`) originally lived on the Daily view only. The later "By session" view was built afterward from scratch, keyed off `track.sessions` rather than a date comparison, and never got that same filter - so viewing the page on the Sunday of a race weekend hid Saturday's forecast in Daily mode while By session still showed it. The 09-07 fix resolved the inconsistency by dropping the filter from Daily too (both views always show the full weekend) - shipped, then explicitly rejected on review ("it should not be shown on a sunday"): Saturday's forecast disappearing once it's over is the correct, intended behavior, not the bug. Re-fixed the other direction instead: restored Daily's `date >= today` filter, and added the equivalent to By session (`weatherDays` now also filters out a day whose real date - `track.startDate` for SAT, `track.endDate` for SUN, this file's own existing SAT/SUN-to-date convention - is before `today`). Both views now consistently hide a day once it's passed.
...
**Rules-of-Hooks violation fixed (2026-09-06):** a round-only deep link (or notification) can render with `track` still `null` before the mount effect resolves it via the live calendar/results fetch. The early `if (!track) return null` guard sat before roughly 15 hooks, so the very next render - once `track` resolved - suddenly ran hooks that never ran on the first render, a genuine Rules-of-Hooks violation, not just a lint warning. Moved the early return to after the last hook call and guarded every `track.xxx` access above it with optional chaining, so hook count and order never differ between renders.
...
**Text-scaling fixes (2026-09-11):** confirmed live on a Pixel 10a with the system font size turned up. `StatBox`'s value `Text` had no `numberOfLines`/`adjustsFontSizeToFit`, so a long single-token value like a 'Filipino-British' nationality or 'Main Championship' class forced a mid-character line break ('Britis'/'h') instead of shrinking to fit - now `numberOfLines={2}` + `adjustsFontSizeToFit` + `minimumFontScale={0.6}`. Separately, the season-history badge row (pts/W/P/PL/FL/DNF pills) had no `flexWrap`, so at large font sizes the combined badge width overran the card's right edge instead of wrapping to a second line - `historyBadges` now sets `flexWrap: 'wrap'`.
...
- Progression Chart (`ProgressionChart` component) - SVG line chart of points accumulation per round. **Standings reconciliation added 2026-09-06:** a standalone championship-points penalty (docked outright, not tied to any single race's own classification) only ever shows up in the official TSL standings total - a pure per-race points sum can never see it, so a penalised driver's chart and stats-tab rank silently drifted above their real position (confirmed live: Adam Morgan's raw sum undercounted his official total, both overstating his chart trajectory and flipping his STATS-tab rank below Rowbottom despite officially sitting above them). `computeProgression(rounds, standings)` now overrides each driver's *final* chart point with their official points total (earlier rounds are untouched); new `reconcileStatsOrder(stats, standings)` re-sorts the STATS tab by the same official totals without changing the displayed W/POD/POL/FL/DNF counts. Mirrors an override `SeasonTable.js` already applied to the results table.
...
**Position-label text-scaling fix (2026-09-11):** confirmed live on a Pixel 10a with the system font size turned up - the position cell has a fixed `width: 36` with no `numberOfLines`, so a `DNF`/`DNS`/`DQ` label wrapped mid-word ('DN'/'F' stacked on two lines) instead of shrinking to fit. Now sets `numberOfLines={1}` + `adjustsFontSizeToFit` + `minimumFontScale={0.6}`.
...
**"&amp;" showing literally in episode titles, fixed (2026-09-07):** Buzzsprout doesn't wrap `<title>` in CDATA (unlike `<description>`/`<content:encoded>`), so any title containing "&" comes through the RSS feed XML-escaped as `&amp;`. `parseRSS()`'s regex extraction just `.trim()`'d the captured group with no entity-decode step, so the raw `&amp;` reached the episode list, the lock-screen "now playing" title (via `useRadio().play()`), and the "New Podcast" push notification body verbatim. Fixed by running the existing `decodeEntities()` (`src/api/parsers.js`, already used for news articles and TTS text) over the extracted title. The Cloud Function side (`sessionNotifications.js`'s podcast-alert check, below) had the identical gap with no equivalent helper available in `functions/` - added a copy of `decodeEntities()` to `functions/shared.js` rather than sharing an import, since `functions/` is a separate CommonJS package from the RN app.
...
- Totals: Championships, Wins (source: btcc.net - 287 drivers as of 2026-09-15, including 104 historical pre-2004-era/title-holder entries)
...
**Totals tabs merge real + historical entries per driver, fixed 2026-09-15:** a driver spanning both eras has two `records.json` rows - a real 2004+-only entry and a `historical: true` override entry carrying the authoritative full-career wins/championships total (see `scrape_btcc_stats.py` below). The Totals tabs previously showed both rows unfiltered and un-merged, so any such driver was listed twice at two different, both-wrong-alone win counts - confirmed live for James Thompson (10 vs 36), Yvan Muller (11 vs 36) and Anthony Reid (3 vs 15). Now grouped by driver name before sorting: a single-entry driver passes through unchanged, a real+historical pair merges into one row using the historical entry's wins/championships (the real entry's other fields, like `driver`'s name spelling, are kept) - the Rates tabs are untouched by this, since they already filter out every `historical` entry regardless.
...
**PartnersScreen** ([src/screens/PartnersScreen.js](src/screens/PartnersScreen.js))

Reached via MoreScreen's "Partners & Sponsors" row. Renders `data/partners.json` (bundled snapshot, swapped for a live `fetchPartners()` fetch only if that returns something non-empty - same pattern as every other bundled-then-live-refreshed dataset in this app) - 8 championship-wide series partners (Kwik Fit, Goodyear, Liqui Moly, BARC, Motorsport UK, Autocar, Dread, AlcoSense), each a card with logo/role/name/description/"Visit website" link.

**All 8 logos hotlinked dead URLs (found live 2026-09-12):** `partners.json` was last edited 2026-07-17, two weeks before btcc.net's WordPress→Vercel migration (2026-07-31) - every `logo` field pointed at the old `/wp-content/uploads/...` path, which no longer resolves at all (confirmed live: opening one directly in a browser returns btcc.net's own "PAGE NOT FOUND"), so the whole screen rendered as blank white cards. Unlike every other image type this app displays (news, drivers, circuits, gallery), `partners.json` has never had a scraper of its own - a hand-maintained file with no mirroring pipeline to catch a source URL going stale. Fixed as a one-time manual mirror (not a new scraper, per explicit scope decision that day): each logo was located on btcc.net's live `/partners/` page and downloaded directly, verified visually, then committed into `data/media/partners/<id>.<ext>` with `partners.json` repointed at `https://raw.githubusercontent.com/yacobwood/BTCC/main/data/media/partners/...`, matching the `MEDIA_RAW_BASE` convention every other mirrored image type already uses. Kwik Fit's own dedicated per-partner section on that same live btcc.net page had an independently broken image (unrelated to the migration, still broken on btcc.net's own site as of this writing) - its logo was instead sourced from a small site-wide logo-wall grid further down the same page, where it rendered correctly. Two regression tests added (`__tests__/screens/PartnersScreen.test.js`) assert no partner logo ever hotlinks `btcc.net` directly and every one resolves through `raw.githubusercontent.com`, to catch a future manually-added partner reintroducing a raw hotlink.
...
- Podcast alerts: polls Buzzsprout RSS, compares `guid` to Firestore state. Sends to `podcast_alerts`. Title is run through `decodeEntities()` (`functions/shared.js`) before sending - see the "&amp;" fix note under [§6 PodcastsScreen](#6-screens-reference).
...
**Scraper failure alerting:** `reportScraperFailure` (HTTP, `SCRAPER_SECRET`-gated) lets the GitHub Actions scraper workflows report into this same pipeline, since a failed workflow run has no way to email on its own. Every scraper workflow (extended 2026-09-06 to cover the last holdouts - `race-day-start`, `scrape-penalties`, `scrape-youtube`) POSTs to it only from a final `if: (failure() || cancelled()) && is_retry` step, not on every failure - see [§20](#20-python-scrapers)'s "Failure handling convention" for the auto-retry-before-alerting mechanism. Either way it calls `logError` with `key: scraper-<workflow>` - same dedup-until-resolved behaviour as the per-minute checks above, and shows up in the same admin ERRORS tab.
...
**One bare in-process retry added 2026-09-13:** confirmed live that GitHub's own `workflow_dispatch` endpoint intermittently returns a plain HTTP 500 ("Failed to run workflow dispatch") for no reason on this repo's side - 5 dispatches failed this way across one 30-minute window, each confirmed via the Actions API run history to have simply never created a run, while every 2-minute tick immediately before and after succeeded normally (self-healed on its own within one cycle either way). Before this fix each blip independently emailed `btcchub@gmail.com` (`logError`'s `alert: true` had no dedup `key`, so it fired every single call) even though no results data was ever actually lost - same noise-without-signal problem already fixed elsewhere for `scrape-results.yml`'s own retry step (2026-09-05) and every Scrapfly fetch (2026-09-11, #36). `triggerResultsScrape` now retries the dispatch once (3s delay) before giving up and only alerts (now with `key: 'triggerResultsScrape'` so a genuinely sustained outage collapses into one alert instead of one per tick) once the retry has also failed.
...
2. `.github/workflows/scrape-penalties.yml` runs 07:05 UTC every Monday (the morning after a Sat/Sun round), gated by `.github/scripts/is_day_after_race.py` (checks `calendar.json` for a round whose `endDate` was yesterday)
...
**SwipeableTabs** ([src/components/SwipeableTabs.js](src/components/SwipeableTabs.js)) - `PagerView`-based tab component. Supports `lazy` mode (only renders the active page on first visit). Used in RoundResultsScreen for session tabs. Also used for the Season screen's DRIVERS/TEAMS/RESULTS/STATS/TABLE/CHART/GALLERY tab bar. Tab label `Text` sets `numberOfLines={1}` + `adjustsFontSizeToFit` + `minimumFontScale={0.6}` (fixed 2026-09-11) - without it, a label like 'RESULTS' or 'GALLERY' wrapped to a second line at large accessibility font sizes and spilled out of the tab row's fixed `height: 44`, over the screen content below (confirmed live on a Pixel 10a).
...
That extraction fixed the *client-side* half of the mechanism, but `displayAndroidDataNotification` still bails entirely (`if (!data?.title) return;`) whenever a sender's payload never put `title` in `data` to begin with - a server-side gap that survived it. Found for real 2026-09-09 after a user reported being `@mentioned` in Live Chat with no push received: `onChatMention` (`functions/chat.js`) sent `notification:{title,body}` + a bare `data:{type:'chat'}`, so a mention landing while the recipient's app was foregrounded (the likely state, since being tagged in chat basically requires being active in it) was silently dropped with zero error on either side. Auditing every other `messaging.send()` call found the identical gap on three more senders in `functions/sessionNotifications.js` (the 15-minute session-starting alert, `weekend_preview`, `standings_update`) and `results_teaser` in `functions/scraperAdmin.js` - `newsCheck.js`'s news sender and the hub/podcast senders already mirrored `title` correctly, which is why only these five went unnoticed. Fixed by mirroring `title`/`body`/`channel` into `data` on all five (merged with whatever type-specific deep-link fields already lived there), redeployed via `firebase deploy --only functions:onChatMention,functions:sendSessionNotifications,functions:notifyResultsUpdate`. The existing `onChatMention` test had used `expect.objectContaining` and only checked `notification.title`, so it passed throughout even with the bug present - a cautionary example for any *new* notification-only sender: assert the actual `data` shape, not just `notification`.

**A third, unrelated bug found 2026-09-11 after the above fix still didn't resolve a live report of "still not getting any chat tag notifications either app open or not":** the recipient's message never reached the send step at all. `resolveMentionedAuthorIds()` only matches `@Name` against entries in `/chat/authorNames` - it has no fallback to a message's own denormalized `authorName` field the way client-side `resolveAuthorName()` does. `/chat/authorNames/{authorId}` is written by `saveChatDisplayName()` (`src/utils/chatIdentity.js`) exactly once, at the moment a name is (re)saved, with a deliberately best-effort `.catch(() => {})` - "the new name still applies going forward via the caller's own local state," per that function's own comment. That's true for *display* (every message the account sends afterward still shows the right name, sourced from local state), but if that one write ever silently fails - offline, a rules hiccup, or the entry never sticks for any other reason - the account becomes permanently un-mentionable with zero visible symptom on either side: their own messages look completely normal, and the Cloud Function's `if (mentionedIds.length === 0) return;` early-exit produces no log, no error, no crash. Confirmed live on production data: `/chat/authorNames` was missing the entry for an account with weeks of message history under a clearly custom, deliberately-chosen name, despite that account still holding a valid, currently-registered FCM token for chat mentions at `/chat/deviceTokens/{authorId}` - proving the token/delivery side was never the problem, only the name→authorId lookup the mention matcher depends on. Immediate fix: manually re-wrote the missing `/chat/authorNames` entry via `firebase database:set` (reversible, matches the account's own known-correct name). Durable fix: added `syncChatAuthorName()` (`src/utils/chatIdentity.js`), called from `ChatScreen.js`'s identity-init effect every time chat is opened - not just at save time - mirroring how `syncChatMentionToken()` already re-registers the device token "once whenever chat identity resolves" (`src/store/settings.js`) rather than only on first setup. This closes the underlying class of bug (a silently-dropped one-shot write with no retry) for every account, not just the one that happened to get caught.

**profanityFilter.js** - Checks input text against the `blacklist.json` word list. Used in ChatScreen and BugReportScreen. **Obfuscation hardening added 2026-09-06:** a plain substring check was trivially bypassed by spacing a word out ("f u c k"), inserting punctuation within it (`. - _ *`, e.g. "f.u.c.k"), or common leetspeak digit/symbol substitutions ("5hit", "a55"). Now normalizes each whitespace-delimited token (stripping intra-word punctuation, mapping `0/1/3/4/5/@/$`) and separately collapses runs of single-character tokens before normalizing those too, so a spaced-out word is reconstructed first. Deliberately conservative - no Unicode confusable/homoglyph detection - and scoped per token so it never merges separate real words across whitespace (e.g. "at 3pm" or "at 5, hit me up" stay clean).
...
**Current: full Scrapfly migration (2026-09-01, completed 2026-09-09).** Every btcc.net-facing scraper (`scrape_news.py`, `scrape_articles.py`, `scrape_calendar.py`/`scrape_full_timetable.py`, `scrape_team_stats.py`, `scrape_btcc_stats.py`, `scrape_gallery.py`, and finally `scrape_circuit_images.py`) now fetches via Scrapfly's paid Scrape API (`tools/scraper/scrapfly_fallback.py`'s `fetch_via_scrapfly`/`fetch_image_via_scrapfly`/`fetch_image_smart`) instead of local Playwright - `asp=true` clears the Vercel challenge from *any* IP, confirmed live, so every workflow now runs on GitHub-hosted `ubuntu-latest` with `SCRAPFLY_API_KEY` as the only new secret required. `scrape_circuit_images.py` was the last holdout (a manual-only script, not called by any workflow at all) - migrated 2026-09-09 rather than give it a brand-new GitHub Actions workflow that still depended on a self-hosted runner. The self-hosted runner registration and `btcc_playwright.py`'s `RenderedFetcher`/`fetch_rendered` are deliberately left completely intact and unused in the repo - dormant, not deleted, in case Scrapfly ever needs to be swapped back out.

**`success=false` guard added to `fetch_via_scrapfly` (2026-09-06):** Scrapfly can return HTTP 200 with `{"result": {"success": false, "error": {...}}}` when the *target* request itself failed server-side - confirmed live 2026-09-02 for `fetch_image_via_scrapfly`, which already guarded against it. `fetch_via_scrapfly` (the plain-HTML fetch path every text scraper uses) did not have the same guard, so a target-side failure disguised as a 200 could be handed to callers as if it were real page content. Now checks `result.get("success", True)` and raises, bringing both fetch paths to the same standard.

**`wait_for_selector` support added to `fetch_via_scrapfly` (2026-09-09):** without it, `fetch_via_scrapfly` only trusts Scrapfly's own generic "page looks done" render-completion heuristic - real runs of `scrape_driver_media.py` hit several intermittent failures all consistent with that heuristic firing before the specific element a caller then tried to extract a `src` from had actually rendered (a headshot fetch 422ing, a car image that "fetched successfully" but decoded to garbage bytes, a recurring 404 on a malformed-looking redirect target). `wait_for_selector` is Scrapfly's own documented, more targeted alternative - a CSS/XPath selector (or `xhr:`-prefixed pattern) the response is held until. `scrape_driver_media.py` passes `.driver-profile-cutout` (every established driver has a headshot, so this is safe to require without risking a driver's genuinely-missing car photo or number graphic turning into a hard timeout instead of that script's own already-graceful "not found" handling); `scrape_shorts.py` passes `.short-card` for the same reason. Only worth setting for an element a page is expected to reliably have - see `fetch_via_scrapfly`'s own docstring for the tradeoff.

**One bare in-process retry added to all three fetch functions (2026-09-11):** before this, the only retry mechanism for any Scrapfly failure was waiting for the next scheduled cron tick (5 minutes, in-hours) - confirmed live the same day: chasing one article's hero/gallery images through three separate flaky failures (a page-fetch `HTTP 422`, a malformed-redirect `404`, an empty-body "success") took several manual `gh workflow run` dispatches across ~10 minutes, when each failure very likely would have resolved within seconds on a plain retry (matches this file's own long-standing note: "one genuinely timed out at 30s that then succeeded in under 7s moments later"). `fetch_via_scrapfly`, `fetch_image_via_scrapfly`, and `fetch_image_smart`'s plain Supabase branch now each retry once (`_MAX_ATTEMPTS = 2`, a 3s delay between attempts) before actually giving up for that cycle - failed Scrapfly requests bill no credits (confirmed via their own billing docs), so this costs nothing when everything's fine. `fetch_image_via_scrapfly` and the Supabase branch also now treat a genuinely empty response body as a failure worth retrying (not just something for `media_utils.save_mirrored_image` to reject afterward - see that function's own 2026-09-11 entry above) - the exact shape of that day's 0-byte image capture.
...
**scrape_tsl.py** - Main results and grid scraper. Fetches TSL timing PDFs for each session (not btcc.net, so unaffected by the Vercel-challenge situation above). Parses race results and starting grids. Writes to `results{year}.json`. Non-finisher results carry `pos: 0`; disqualifications additionally carry `status: "DQ"`. At the end of each run it also updates circuit lap records in `calendar.json`, writes `standings.json` (see below), and triggers `compute_records.py`. (Team stats used to run here too - see `scrape_team_stats.py` below for why that moved out.) Every PDF suffix (`SESSION_SUFFIXES`, `GRID_SUFFIXES`, `CHAMPIONSHIP_SUFFIX`) gets `trg` appended at the point of use (`f"{suffix}trg"`) - TSL's touring-car category disambiguator, needed because a single TOCA event ID covers BTCC plus several support series sharing the same file namespace.

`standings.json`'s official points/wins come from a genuinely separate TSL PDF (the championship "ptstrg" PDF, via `parse_championship_pdf()`), not derived from `results{year}.json`'s own race-by-race data - TSL doesn't publish this PDF until after a round's Race 3, so mid-round this falls back to the previous round's official PDF rather than computed standings (`compute_standings_fallback()` only kicks in if no PDF is found at all, for any round). This is a real, external timing gap independent of this repo's own commit cadence: for a window after each round finishes (until TSL actually publishes that round's `ptstrg`), `results{year}.json`'s own per-race point sum can legitimately run ahead of `standings.json`'s still-catching-up official total - not a bug, and not something any commit-orchestration change here can close. `ResultsScreen.js`'s `computeProgression`/`reconcileStatsOrder` (see [§6](#6-screens-reference)) exist specifically so the app shows one consistent number everywhere during that window (always deferring to `standings.json`'s official total) rather than two competing ones.

**Spurious-commit fix added 2026-09-06:** unrelated to the above - `standings.json`'s `updated` field used to get re-stamped with the current time on every single scrape tick regardless of whether anything else in the file actually changed, so `git-auto-commit-action` committed it on essentially every 2-minute tick during a raceday (confirmed live: round 7/Donington GP weekend logged 193 commits to this file, all but ~8 of them differing from their predecessor only in this one field - real content changes, when they happen, already land in the same commit as `results{year}.json`'s own change, confirmed across every genuine case checked). `merge_standings_timestamp()` now reverts `updated` back to the existing on-disk value whenever nothing else about the standings data changed, keeping a genuine no-op tick byte-identical to what's already committed.
...
**Empty-result guard added 2026-09-06:** `scrape_round_penalties()` returning `[]` is ambiguous on its own - a genuinely penalty-free round and a BARC markup change that made the parser silently match nothing look identical from an empty list alone, and the old code wrote either straight over any existing data. An empty result is now only accepted when nothing was already on file for that round; otherwise it's logged as a suspected parse failure, the existing data is left untouched, and the run exits non-zero so the retry/alert path fires instead of quietly losing real penalty data.
...
**scrape_shorts.py** (added 2026-09-08) - Mirrors btcc.net's own homepage "Latest BTCC Shorts" carousel into `data/shorts.json`, triggered from the admin panel's Scrapers tab (SHORTS card) or weekly (Wednesday 09:33 UTC via `scrape-shorts.yml`). **The first scraper in this repo to fetch btcc.net's bare root URL** - every other one targets a specific subpath (`/news/`, `/calendar/`, `/driver/<slug>/`, etc.). Confirmed live 2026-09-08 (real markup, via a screenshot + copied outerHTML) that each short is `<a class="short-card" href="https://www.youtube.com/shorts/<id>">` with a thumbnail at `https://i.ytimg.com/vi/<id>/hqdefault.jpg` - YouTube's own public CDN, not signed, not expiring, and not behind btcc.net's Vercel bot-challenge at all (unlike article/driver/circuit images, which route through btcc.net's own `/api/media/<uuid>` redirector), so - same reasoning as `scrape_gallery.py`'s own hotlinked photos above - the thumbnail is derived directly from the video ID rather than scraped separately or mirrored as image bytes. Each run replaces `shorts.json` wholesale rather than merging, since this mirrors a rotating "latest N" widget, not an accumulating archive. In the app, `NewsScreen.js` renders these as a freely-scrolling horizontal thumbnail rail (not the paged/full-bleed treatment `TrackDetailScreen.js`'s `PhotoCarousel` uses for its own gallery photos - every short is equally-weighted and tapping any of them exits to YouTube either way) with no dedicated screen of its own - deliberately simpler than an original Digests/Explainer-style teaser-into-a-full-screen design, since there's no "detail view" a dedicated screen would add for content whose entire interaction is "tap to open externally."
...
**Per-race points override write-ordering bug, found+fixed 2026-09-09**: `_parse_ptstrg_per_race()` (coordinate-based, derives QR/Race 1/2/3 column x-positions dynamically from the PDF's own header elements rather than hardcoded per-layout-version bounds - see `_build_results_output`'s own docstring for the specific PDF-format history this superseded) and `_apply_per_race_points()` (which overrides each scored session's locally-computed points in `results{year}.json` with the authoritative value straight from this same championship PDF) had both been correct all season - the bug was in `main()`'s own sequencing. `results{year}.json` used to be serialized and written to disk immediately after `output_rounds` was assembled, and only *afterward* did the championship-PDF fetch run and call `_apply_per_race_points()` - which correctly overrode `output_rounds`' per-race points in memory, but nothing ever wrote that corrected state back to disk a second time. Every per-race point value in `results{year}.json` was therefore always the locally-reconstructed one - itself a real, working computation (base points-per-position table plus this file's own fastest-lap/led-a-lap bonus detection, re-verified live 2026-09-09 by re-fetching every round's actual TSL PDF and re-running that exact bonus logic against it - zero discrepancies found there), just a fundamentally less authoritative source than the officially-published breakdown, and the two were free to drift apart over a season with no override ever actually landing to stop it. Confirmed live: summing `results2026.json`'s own points per driver mismatched `standings.json`'s official total for 5 drivers (Sutton/Ingram/Cook -1, Chilton +1, Lloyd +10 - the last being a fresh mid-season return, not the same drift class as the other four). Fixed by extracting `_build_results_output(year, output_rounds, per_race, scored_sessions)` - it applies the override and builds the dict to write in one function, so the ordering is enforced by construction rather than trusted to two separately-sequenced steps in a 400+ line `main()`; `main()` now calls this once, after the championship-PDF loop has run (or safely no-op'd with empty `per_race`/`scored_sessions` defaults if every fetch attempt failed), and writes its return value - never `output_rounds` serialized any earlier. Affected historical rounds still need a fresh scrape to actually backfill the correction on already-committed data - this fix only stops the drift from recurring going forward.
...
**Two bugs fixed 2026-09-06:** `title_case()` used `str.capitalize()` per word, which only lowercases everything after the first character of the *whole* token rather than respecting `-` as its own word boundary - `"TAYLOR-SMITH".capitalize()` produced `"Taylor-smith"`, not `"Taylor-Smith"`, silently splitting Áron Taylor-Smith's career stats across two rows in `records.json`. Switched to `str.title()`. Separately, loading the existing `historical` (pre-2004) entries used to swallow any parse failure (`except Exception: pass`) and silently continue with an empty list, which would have permanently dropped every pre-2004 entry on the next write if `records.json` was ever transiently corrupt - now extracted into its own `load_historical_entries()` and made fatal (`sys.exit(1)`) instead.

**Qualifying Race excluded from Championship-Round stats, 2026-09-15:** cross-checked `records.json` against insidebtcc.com/drivers/ (a 255-driver all-time BTCC stats table) - `build_timeline()`'s per-round label list included "Qualifying Race" (which sets the Race 1 grid) alongside Race 1/2/3, so `compute_records()` counted it as a full Championship Round in every stat. Every current driver's `starts` came out higher than that external reference by roughly how many Qualifying Races they'd started (e.g. Dan Cammish 265 vs 228, James Dorlin 54 vs 39); wins/podiums/poles/fastestLaps were inflated the same way for several drivers too. Fixed to match `career_stats.py`'s own regulation-verified scope (reg 1.6.2.a / 1.6.9-1.6.10, see that entry below): Qualifying Race results still count toward `points` (a real points-paying session) but are now skipped entirely for starts/wins/podiums/poles/fastestLaps/dnfs and the streak trackers - not counted as a non-win/reset either, simply invisible to those stats, matching how `career_stats.py`'s `PODIUM_SESSIONS` already worked. `data/records.json` regenerated the same day; per the standalone-safety note above, the official wins/championships overrides that regeneration would have reverted were manually reapplied from the last known-good scrape rather than left to self-heal, since a live re-scrape wasn't available in-session (Scrapfly needs a key this environment doesn't have).
...
**scrape_btcc_stats.py** - Weekly official-stats override pass. Fetches all-time wins (`btcc.net/history/statistics/drivers/`) and championship counts (`btcc.net/history/champions/btcc-titles/`) and patches them into `records.json` on top of whatever `compute_records.py` last computed - this is what makes pre-2004 career totals correct. The wins/titles content itself turned out to still be WordPress-migrated static text and Gutenberg tables even after the Vercel migration, so `parse_wins()`/`parse_titles()` needed no changes - only the fetch mechanism did. Runs Monday 06:04 UTC via `scrape-btcc-stats.yml` (dropped from daily - btcc.net's own dev confirmed results/standings only move on race weekends, so a weekly check reliably catches anything new; moved off the exact :00 mark 2026-09-09 after `scrape-gallery.yml` was confirmed live to silently skip a scheduled tick while sitting on it - see this section's own cron-gap entry above and project_gallery_croft_missed_schedule_fix memory).

**scrape_calendar.py** - Parses the BTCC calendar to update `calendar.json` with round dates and venues (internally calls `scrape_full_timetable.py` per round to populate support-series timetables into each round's `fullTimetable`). The calendar page's card order is **not chronological** (a grid/layout artifact on the live site) - rounds are sorted by parsed date before round numbers are assigned, rather than trusting encounter order. Also note: btcc.net abbreviates September to 4 letters ("SEPT") while every other month uses 3 - the date regex allows `{3,4}` specifically to avoid silently dropping September rounds (see `test_scrape_calendar.py`). Runs weekly (Monday 09:06 UTC via `scrape-calendar.yml`, immediately followed by `scrape_schedule.py` and `merge_schedule.py` in the same job; moved off the exact :00 mark 2026-09-09, see project_gallery_croft_missed_schedule_fix memory) rather than daily - btcc.net's own dev confirmed the calendar "doesn't get updated often, once set at season launch kinda stays that way most of the time".
...
**scrape_articles.py** - Mirrors btcc.net articles in full (not just the single latest headline `scrape_news.py` tracks) into `data/articles/page_<n>.json` (PAGE_SIZE=20 each, must match `fetchArticles()`'s `perPage` in `src/api/client.js`) plus `data/articles/index.json` (slug → page number). Gets slug/title/excerpt/date/image for every card from the rendered `/news/` listing page, then fetches each article's own page for full body HTML - but only for slugs not already cached, so a steady-state run is typically 0-1 extra page loads, not a full re-fetch of every article. If a card's listing-page image is missing, that per-article page fetch also checks the page's own `og:image` meta tag as a fallback (added 2026-09-04 - see this section's other `scrape_articles.py` entry above for the full writeup). Each run's freshly-scraped cards are merged into the full previously-accumulated archive (not just replaced), then capped at MAX_ARTICLES=500 by date - this per-page split is what lets the app fetch one page's worth of data regardless of how deep the archive goes; an earlier version wrote one flat `articles.json` capped the same way, but that meant every fetch (list, search, or a single slug lookup) downloaded the entire archive's full content every time. `--backfill-pages N` crawls `https://btcc.net/page/2/` onward (page 1 is the bare `/news/` URL) for a one-off deep backfill (not for routine runs) - confirmed live 2026-09-01 this genuinely reaches real, chronologically-ordered content all the way to page 201 (~November 2013), once the real pagination URL was found (see §20's Scrapfly migration writeup - an older version of this scraper had been testing `/news/page/<n>/`, which 404s, and never actually reached past ~3 pages as a result). (The old WordPress `/feed/` RSS source this used for full content no longer exists post-migration.) Runs every 5 minutes in the same `scrape-news.yml` workflow as `scrape_news.py`. **Gallery image mirroring added 2026-09-11:** btcc.net's custom "btcc-gallery" block (a grid of photos inline in an article's own body, distinct from its one featured/hero image) emits each photo as a root-relative `/site-assets/<path>` src - a shape `MEDIA_SRC_RE_FRAGMENT` didn't recognise before this, so these were never mirrored at all, just hotlinked live from btcc.net in the app's `ArticleScreen` WebView. Confirmed broken on-device for two articles seven weeks apart (25 Jul "PM1 Junior prototype car unveiled" and 11 Sep "BTCC visit Darlington Memorial Hospital...") - silently broken since at least July, not a new regression, just unnoticed because those articles still had a separate working hero photo up top. `mirror_gallery_images()` mirrors every such image the same way the hero image already is and rewrites `content.rendered` in place; bounded by the same `IMAGE_RETRY_WINDOW` (`needs_gallery_mirror()`) so a permanently-dead gallery photo doesn't re-bill Scrapfly every 5-minute cycle forever, except a just-fetched article's body always gets one attempt regardless of age. `prune_orphaned_images()` was also updated to treat a gallery image referenced from `content.rendered` (not just `_embedded`) as still-referenced, since otherwise a freshly-mirrored gallery image looked orphaned and got deleted on the very same run. Same investigation added a **3rd hero-image fallback tier**: an article with neither a listing-card thumbnail nor an `og:image` (confirmed live: "BTCC visit Darlington Memorial Hospital...", a "community visit" piece with only an inline gallery, no dedicated featured photo at all) now falls back to its own first successfully-mirrored gallery image as the hero, rather than rendering a blank hero indefinitely. `ArticleScreen.js`'s injected WebView CSS also gained the gallery block's own grid layout rules (previously entirely unstyled, so even a correctly-loading gallery image would have stacked full-width one per row rather than as a grid). **`heroSource` marker added same day, same session:** the tier-3 fallback above turned out able to be flat wrong - confirmed live when the user compared this exact article against the real btcc.net page directly and it showed a different, correct hero photo, meaning a real `og:image` did exist and this run's fetch of it had simply failed transiently (the same class of live per-request Scrapfly flakiness hit twice elsewhere the same day - see the empty-body guard entry above). Without a marker, a wrong gallery-derived guess is indistinguishable from a confirmed real image on every later run (both are just a `MEDIA_RAW_BASE` URL), so `prior_image and prior_image.startswith(MEDIA_RAW_BASE)` alone would keep it locked in forever even once the real image became fetchable again. `_embedded.heroSource = "gallery"` (absent for a confirmed card/og image) now also feeds `image_retry_needed`, so a gallery-sourced hero keeps getting one more full-page fetch per cycle (bounded by the same `IMAGE_RETRY_WINDOW`) until either the real image is found (clearing the marker) or the window lapses.
...
**Detection-failure guard added 2026-09-06:** when green-screen detection genuinely fails (`detect_race_timestamps()` returns `(None, None)`), `main()` used to fall back to writing placeholder `t1,t2,t3 = 0,0,0` URLs anyway. Since `build_urls()` never returns empty strings, `find_target_round`'s "already complete" check (`len(urls) >= 6 and all(urls[3:6])`) then saw the round as permanently done and never retried it - wrong Race 2/3 deep links stuck in place forever. A genuine detection failure now leaves the round's existing `youtubeUrls` untouched and exits 0 (retry later) without calling `save_results` at all. `scrape-youtube.yml` picked up the supporting infrastructure this needs to be safe now that a run can legitimately do nothing: a `concurrency` group (so an overlapping manual "catch late uploads" re-run queues rather than racing a scheduled run), a `git pull --rebase` before scraping and `--force-with-lease` on push (so a queued/retried run isn't working from stale HEAD), and the same retry-once-before-alert pattern the 5 btcc.net-facing workflows already use (this script doesn't touch btcc.net directly, but a weekly-only schedule means a bare failure would otherwise wait a full week for its next natural attempt). Also switched the `YOUTUBE_COOKIES` secret from being echoed directly into the shell command to being passed via `env` + `printf`, safer against special characters.

**scrape_team_stats.py** - Fetches race/win totals per team from `btcc.net/team/<slug>/` and writes them into `data/drivers.json`. Used to run as a sub-step of every `scrape_tsl.py` invocation (every 2 minutes during a live race weekend), but that meant launching a headless browser on every tick just to re-check totals that only change once a weekend's results are final - now runs on its own weekly schedule via `scrape-team-stats.yml` instead (Monday 06:34 UTC, same reasoning as `scrape_btcc_stats.py` above; moved off the exact half-hour mark 2026-09-09, see project_gallery_croft_missed_schedule_fix memory). Used to also mirror each team's card-background graphic and car photo from `/teams/` into `cardBgUrl`/`carImageUrl` - that image-fetching code was stripped out 2026-08-18 (see below), leaving this script purely a stats scraper.

**Zero-stats guard added 2026-09-06:** `_fetch_team_stats()` returns `{}` (not an exception) when `STAT_RE` stops matching a page that otherwise fetched fine - e.g. a btcc.net markup change - which previously silently re-wrote every team's existing `totalRaces`/`totalWins` with nothing new while still logging a normal "updated" line. Now fails the run (`sys.exit(1)`) if every team page fetched OK but none of them parsed any real stats, mirroring `scrape_btcc_stats.py`'s own guard for the same failure shape.

**scrape_circuit_images.py** - On-demand only (no schedule), triggered from the admin panel's CIRCUIT IMAGES card via `scrape-circuit-images.yml`. Fetches each circuit's hero photo from its `btcc.net/circuit/<slug>/` page and saves it into `data/media/tracks/`, repointing that track's `imageUrl` in `tracks.json` at the GitHub-raw-hosted copy. Needed because the new site serves this image from a private Supabase Storage bucket behind a per-request signed URL (expires within the hour) via btcc.net's own stable `/api/media/<uuid>` redirector - same mechanism the now-archived `scrape_driver_images.py` used to mirror for driver cutouts, just a different page/selector (`.circuit-profile-hero`'s `background-image` `url(...)` instead of an `<img src>`). `layoutImageUrl`/`raceImages` are deliberately not touched - every current track has a bundled SVG in `TrackDetailScreen.js`'s `BUNDLED_TRACK_LAYOUTS` that takes priority, and the race-photos carousel is never actually pushed into that screen's render list, so both are dead code regardless of URL validity. Brands Hatch GP is skipped - its `imageUrl` already points at `images.msv.com` (the circuit's own site) and still resolves. Migrated 2026-09-09 from `btcc_playwright.RenderedFetcher` to Scrapfly (`fetch_via_scrapfly` + `fetch_image_smart`, modeled on `scrape_team_stats.py`'s own shape) so it could finally get a real GitHub-hosted workflow instead of needing a brand-new self-hosted-runner dependency - was the last scraper on the old path.
...
- **Scrapers** - One RUN button per scraper, dispatching its GitHub Actions workflow directly via the API. As of 2026-09-09, every scraper in the repo has a card (`results`, `calendar`, `btcc-stats`, `youtube`, `driver-media`, `driver-roster`, `shorts`, `news`, `penalties`, `gallery`, `team-stats`, `circuit-images` - the last one new that same day, see `scrape_circuit_images.py`'s own entry above) - each card also has a lazily-loaded "▾ recent runs" panel (last 5 runs, status/trigger/time-ago) built entirely off the `SCRAPER_WORKFLOWS` array by `initRecentRunsUI()` rather than hand-added per card, so a future new scraper's card gets this automatically too. Deliberately not part of the existing 15-second status-chip poll loop (`refreshAllStatus()`) - a `per_page=5` history list is only fetched on first expand and cached, not re-polled, since most cards will never be expanded in a given session.
