...
**Saturday's Daily card disappearing on Sunday, fixed (2026-09-07):** an earlier fix (hiding a weather card once its day is over) filtered `weather.daily` down to `date >= today` in the Daily view only. The later "By session" view was built afterward from scratch, keyed off `track.sessions` rather than a date comparison, and never got that same filter - so viewing the page on the Sunday of a race weekend hid Saturday's forecast in Daily mode while By session still showed it, an inconsistency reported live. Fixed by dropping the past-day filter from the Daily view too, so both modes now always show the full weekend regardless of which day it's viewed on - matching the behavior By session already had, rather than adding the filter to By session as well and losing that reference view of a day just gone.
