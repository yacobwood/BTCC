...
**Table vs Chart points mismatch, fixed 2026-09-06:** the Season Table showed a driver's official TSL-scraped total (`SeasonTable.js` overrides its own race-by-race fallback with `standings.json`'s `points` whenever it's loaded), while the Progression Chart's `computeProgression()` always independently re-summed every race's own `points` field from `results{year}.json` and never looked at `standings.json` at all - two separately-scraped files with no reconciliation between them. Confirmed live, round 8 (Croft): Ashley SUTTON and Tom INGRAM each showed a small (~1pt) gap between the two tabs from ordinary scrape-timing lag between the two files, but Adam MORGAN's gap was a full 5-6pts and didn't close on a rescrape - his official total carries a standalone championship-points penalty (Sporting Regs judicial decision, not tied to any single race's classification), which only ever exists in the TSL PDF's own running total and can never be reconstructed by summing individual race results, since no race's own points field reflects it. Fixed by having `computeProgression()` accept the `standings` object and override just the *final* point of each driver's series with `standings.drivers[].points` when available (same override `SeasonTable.js` already does for the table) - earlier rounds are left as computed, since the PDF doesn't say which round a standalone deduction applies against. The STATS tab had the same blind spot one level removed: `computeSeasonStats()`'s raw per-race sum was also used to *rank* drivers (the row number shown is index-based), so Morgan's under-counted raw total (226) could outrank him below Rowbottom (231, no penalty) even though the official standings place Morgan above (232 to 231) - fixed via a small `reconcileStatsOrder()` helper that re-sorts the computed stats by the official points when standings are loaded, without touching the displayed W/POD/POL/FL/DNF counts themselves (those come straight from race results and were never wrong). See `__tests__/utils/resultsCompute.test.js`.
