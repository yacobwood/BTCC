...
| News source | GitHub-mirrored btcc.net article snapshot (`articles.json` - see [§19](#19-python-scrapers); btcc.net is now a Vercel-hosted React app with no public REST API, so the app never hits it directly) |
...
| `fetchArticles(page, perPage, search)` | GitHub (`articles.json` mirror) | Paginates and search-filters the mirrored array client-side - btcc.net has no public REST API, so this never hits btcc.net directly (see [§19](#19-python-scrapers)) |
...
| btcc.net (Vercel) | `https://www.btcc.net/news/` + per-article pages | News articles - scraped into `news.json` (latest headline, for the notification trigger) and `articles.json` (latest full articles, for the app's News tab and article deep-links) by `scrape_news.py`/`scrape_articles.py` via headless Chromium (see [§19](#19-python-scrapers)) |
...
| `articles.json` | Latest btcc.net articles in full (title, content, image, category), scraped every 5 minutes so the app's News tab, search and article deep-links can read them without hitting btcc.net directly (it's a Vercel-hosted React app with no public REST API) |
...
**scrape_articles.py** - Mirrors the latest btcc.net articles in full (not just the single latest headline `scrape_news.py` tracks) to `articles.json`. Gets slug/title/excerpt/date/image for every card from the rendered `/news/` listing page, then fetches each article's own page for full body HTML - but only for slugs not already cached in `articles.json`, so a steady-state run is typically 0-1 extra page loads, not a full re-fetch of every article. (The old WordPress `/feed/` RSS source this used for full content no longer exists post-migration.) Runs every 5 minutes in the same `scrape-news.yml` workflow as `scrape_news.py`.
...
**scrape_circuit_images.py** - Manual, one-off mirror tool (not on any schedule). Fetches each circuit's hero photo from its `btcc.net/circuit/<slug>/` page and saves it into `data/media/tracks/`, repointing that track's `imageUrl` in `tracks.json` at the GitHub-raw-hosted copy. Needed because the new site serves this image from a private Supabase Storage bucket behind a per-request signed URL (expires within the hour) via btcc.net's own stable `/api/media/<uuid>` redirector - same mechanism `scrape_driver_images.py` already mirrors for driver cutouts, just a different page/selector (`.circuit-profile-hero`'s `background-image` `url(...)` instead of an `<img src>`). `layoutImageUrl`/`raceImages` are deliberately not touched - every current track has a bundled SVG in `TrackDetailScreen.js`'s `BUNDLED_TRACK_LAYOUTS` that takes priority, and the race-photos carousel is never actually pushed into that screen's render list, so both are dead code regardless of URL validity. Brands Hatch GP is skipped - its `imageUrl` already points at `images.msv.com` (the circuit's own site) and still resolves.

**scrape_driver_cutouts.py** - Manual, one-off refresh tool (not on any schedule). Re-fetches every currently-racing, already-bundled driver's cutout photo from their `btcc.net/driver/<slug>/` page and overwrites `src/assets/driver_images/<number>.webp` directly - for when btcc.net publishes an updated photo for a driver who already has a bundled asset (distinct from `scrape_driver_images.py`, which only backfills the live `imageUrl` JSON fallback for a driver who doesn't have a bundled asset yet). Discovers each driver's slug from the `/drivers/` listing page by name match (same mechanism `scrape_driver_backgrounds.py` uses), so it doesn't need a hardcoded slug map and self-adjusts as the roster changes - a driver who's left the grid (e.g. Max Buxton, still bundled but no longer listed) is simply skipped with a warning rather than erroring, leaving their existing bundled photo untouched. Resizes each fetched cutout to fit the existing ~300x450 bundled convention via Pillow.
