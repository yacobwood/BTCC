...
**Comments shown inline instead of behind a button (added 2026-09-14):** the vote buttons used to be followed by a plain "💬 N Comments" button that opened `CommentsSheet` - live Firestore data checked before this change showed comment engagement was essentially zero (1 comment across 40 sampled recent articles), and a `0`-count button reads as "empty, skip it" rather than inviting anyone in. `buildHtml()`'s `.comments-preview` block replaces it: the newest 1-2 top-level comments (never replies - `buildCommentPreviewItems()`, newest-first, unlike the sheet's own oldest-first order) render directly under the vote buttons, or "No comments yet. Be the first!" when there are none, with exactly two tap targets - the preview body itself (`open_comments`) and a standing "Add a comment" pill (`add_comment`, also fired by tapping the empty state) that opens the same sheet already focused on the input via a new `focusInputOnOpen` prop, the same `inputRef.current?.focus()` the existing "Reply" button already uses, just fired on open instead of on press. The full comment list, threading, name prompt, edit/delete and keyboard handling all stay exactly as they were in the native `CommentsSheet` modal - only the entry point changed, not the composer, since a WebView-embedded text input has its own well-known keyboard/scroll problems the native sheet doesn't. Preview rows are built with `createElement`/`textContent`, never `innerHTML`, since comment text only passes through a profanity filter, not HTML sanitisation - this is the one place in the article WebView rendering literal user-submitted content, so the same escaping-by-construction the native `<Text>` components already got for free needed doing explicitly here. New `article_comments_opened` GA4 event tags which of the three entry points was used (`preview_view`/`preview_add`/`preview_empty`) - posting/reacting/deleting a comment still has no analytics of its own, a pre-existing gap this didn't set out to fix.
