...
Two-tab view: Drivers (card grid with number, photo, team, car class, and the driver's own car livery rotated on its side, running down the right edge beneath the number) and Teams (team cards showing just the sponsor logo). Drivers can be starred as favourites. Tapping navigates to DriverDetail or TeamDetail. A driver whose `currentlyRacing` field in `drivers.json` is `false` (e.g. moved out of their seat mid-season to a reserve/development role) drops out of the main "N CONFIRMED" grid into a separate "NOT CURRENTLY RACING · RACED IN 2026" section below it, and is excluded from their last team's driver roster on TeamDetailScreen - kept visible rather than deleted, since they did race that season.

A driver card's `carImageUrl` (added 2026-08-21) is the driver's own resolved car - not a shared team image - because a team can field more than one livery: Steel Seal with Power Maxed Racing's Dexter Patterson and Nick Halstead each have their own separately-branded car (Halstead's mid-season "Ask GVT" livery, confirmed by an official photo, differs from Patterson's). Laid out as three deliberately non-overlapping zones rather than stacking everything on top of the photo: `driverNumberImg`/`driverNumberBg` stay top-right as before, the driver photo (`driverPhoto`) narrowed from 100% to 60% width (an initial pass at 72% moved further left/bigger by request) and left-aligned (`driverImageArea`'s `alignItems: 'flex-start'`), and the car runs down the right edge in a vertical strip underneath the number, rotated -90 degrees anti-clockwise by request (`driverCarSide`/`driverCarSideImg`) - the number's already up top and the photo's pinned left, so all three elements get their own space with nothing competing. The rotated wrapper is sized in plain pixels (`TILE_W` and the derived `CAR_SIDE_TOP`/`CAR_SIDE_MARGIN`/`CAR_SIDE_W`/`CAR_SIDE_H` constants), not percentages, because a percentage width/height resolves against the element's own unrotated box before the `transform` is applied - the image needs its width and height swapped ahead of the rotation so the *rotated* result lands on the footprint the layout actually reserved, not the pre-rotation one. Went through three earlier layouts first: an absolutely-positioned bottom-left badge overlapping the photo (fine at a small size, but a visible collision with the driver's own legs/feet once sized up per request), a dedicated full-width strip below the photo (avoided the collision, but made every tile noticeably taller), and an upright bottom-right corner badge (`driverCarImg`, 50%/33%, up from an initial 42%/28%) that avoided both problems but capped how large the car could get before it started crowding the number - rotating it onto its side (2026-08-21, by request) let it grow noticeably bigger within the same tile footprint. (`DriverDetailScreen`'s much bigger header uses this exact same rotated-strip arrangement too - see that screen's own entry below.) Team cards dropped the car cutout entirely for the same underlying reason - a single "representative" car on a team tile would just be misleading once more than one livery exists under that team - and instead show the sponsor logo (`logoUrl`) large and centered, filling most of the tile rather than a small top-right badge. Teams with no logo file yet (e.g. CPRL as of 2026-08-20) simply render without one - no placeholder. `MerchScreen.js`'s tiles use the identical large/centered `teamLogoImgLarge` treatment for parity (see below).

The car badge's URL is rewritten to a small pre-generated thumbnail (`carThumbUrl()`, `<name>-thumb.webp` alongside every full-size file in `data/carImages/` - see `scripts/generate_car_thumb.py`), not the full-size original. Root cause, confirmed live via a device log capture rather than guessed: Android's image pipeline decodes to an uncompressed bitmap sized off pixel dimensions, not file size, so every car image - already WebP-compressed to ~90KB on disk - still costs 1536x1024x4 bytes = 6MB of *decoded* memory each. Unlike `cardBgUrl` (shared across a team's drivers, so Android decodes and caches it once), every car image is unique per driver with no reuse to fall back on, so ~23 of them on one non-virtualized grid blew straight through Android's ~192MB decode pool - it looked like "the bottom of the list fails to load" because whichever tiles' decodes landed after the pool was already full just failed outright, confirmed via `onError`'s actual native reason: `Pool hard cap violation`. Two earlier attempts (staggering the badge's mount, widening `CachedImage`'s retry budget) didn't fix it and were reverted - both were trying to fix a request that hadn't gotten a turn yet, when the real failure was a *decode* that couldn't fit in memory no matter how long it waited or how many times it retried. `TeamDetailScreen`'s per-driver car cards use the same thumbnail for the same reason, even though that screen only ever shows 2-4 cars at once (never actually hit the cap) - no reason to decode 15x more bitmap than a couple-hundred-px card needs just because the count happens to be small there.
...
The header (added 2026-08-21) also shows the driver's own car - same underlying reason as DriversScreen's tile - a team can field more than one livery, so it has to be the driver's own `carImageUrl`, not a shared team image. Now uses the exact same rotated-strip arrangement as that tile: `headerNumberImg`/`headerNumber` stay top-right, `headerPhoto` narrowed 100% -> 60% width and left-aligned (`headerBg`'s `alignItems: 'flex-start'`), and the car runs down the right edge underneath the number in a vertical strip rotated -90 degrees anti-clockwise (`headerCarSide`/`headerCarSideImg`, sized off `HEADER_W`-derived pixel constants the same way as the tile's `TILE_W` ones, for the same reason - see that entry above) - this header is much bigger in absolute terms, so the car still reads as a real showcase even confined to a strip. Went through several earlier passes first: a bottom-left overlay (fine when the car was smaller, since there was enough headroom for it to sit at the driver's feet without clipping through them - see below for the sizes tried: 22%/14%, then 48%/32%, then 68%/45%), which stopped working once the car grew large enough to visibly clip the photo, matching the exact problem the tile hit; then an upright bottom-right corner badge (`headerCarImg`, 50%/33%, matching the tile's then-current `driverCarImg` proportions) that fixed the clipping but, like the tile, capped how large the car could get - rotated onto its side (2026-08-21) for the same reason and the same result. Uses the same pre-generated `-thumb` URL (`carThumbUrl()`, a third identical copy of DriversScreen's/TeamDetailScreen's helper) as everywhere else this renders - this screen never actually hit the decode-memory cap that motivated adding it (only one driver's car is ever on screen here), but there's no reason to decode 15x more bitmap than a corner badge needs just because the count happens to be small in this context.

Once the car badge grew into that real showcase size, the top-right number (`headerNumber`/`headerNumberImg`) read as too dominant by comparison and was shrunk to match - `headerNumber`'s plain-text fallback from `fontSize: 160` to `110`, `headerNumberImg`'s branded-graphic box from `60%/48%` of the header down to `45%/36%`.
...
The hero (added 2026-08-21, replacing a single `team.carImageUrl`) shows one car card per current driver - `team.drivers` (already the active, non-reserve roster `parseGrid()` filters to) mapped to each driver's own resolved `carImageUrl`, captioned with their name - rather than one image standing in for the whole team, since a team can field more than one livery (a 2-driver team like Steel Seal with Power Maxed Racing shows 2 distinct cars; a 4-driver team like Team VERTU or NAPA Racing UK wraps into a 2x2 grid). The sponsor logo (`logoUrl`) keeps its original top-right treatment over the hero background unchanged - sized via `aspectRatio` off its own width rather than a height %, now that the hero's height is driven by however many car rows it holds instead of a fixed 2:1 banner, so the logo doesn't balloon for a 4-driver team's taller hero. A driver with no `carImageUrl` of their own (e.g. a reserve with no car cutout yet) simply contributes no card - the hero still renders for its logo/other cars, or collapses to a plain spacer if there's genuinely nothing to show.
