...
The header (added 2026-08-21) also shows the driver's own car - same underlying reason as DriversScreen's tile - a team can field more than one livery, so it has to be the driver's own `carImageUrl`, not a shared team image. Now uses the exact same non-overlapping quadrant layout as that tile: `headerNumberImg`/`headerNumber` stay top-right, `headerPhoto` narrowed 100% -> 60% width and left-aligned (`headerBg`'s `alignItems: 'flex-start'`), and `headerCarImg` (50%/33%, matching the tile's `driverCarImg` proportions) sits bottom-right in the corner that frees up - this header is much bigger in absolute terms, so the car still reads as a real showcase, it just no longer overlaps the photo. Went through several earlier passes first: a bottom-left overlay (fine when the car was smaller, since there was enough headroom for it to sit at the driver's feet without clipping through them - see below for the sizes tried: 22%/14%, then 48%/32%, then 68%/45%), which stopped working once the car grew large enough to visibly clip the photo, matching the exact problem the tile hit and fixed the same way. Uses the same pre-generated `-thumb` URL (`carThumbUrl()`, a third identical copy of DriversScreen's/TeamDetailScreen's helper) as everywhere else this renders - this screen never actually hit the decode-memory cap that motivated adding it (only one driver's car is ever on screen here), but there's no reason to decode 15x more bitmap than a corner badge needs just because the count happens to be small in this context.
