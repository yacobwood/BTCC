...
**Stale `poles` regenerated 2026-09-15:** cross-checked `drivers.json` against insidebtcc.com's per-driver season-by-season pages for 4 current/recent drivers (Tom Ingram, Dan Cammish, Josh Cook, Gordon Shedden) - every `poles` discrepancy found (17 across those 4 alone, e.g. Josh Cook 2022 stored as 4, actually 1) had the identical shape: the stored value was stale, and `compute_year_standings()` run fresh right now already computed the correct figure, matching insidebtcc exactly every time - `wins`/`podiums`/`fastestLaps`/`points` did not show this pattern (a handful of small disagreements existed, but the *stored* value matched a *fresh* recomputation from the same source data, so those look like insidebtcc's own data rather than ours - not touched). Regenerated `poles` for every driver's every `history[]` year from a fresh `compute_year_standings()` call, changing nothing else (41 values changed across the full roster, all decreases). Years with no source file at all (pre-2004, e.g. Gordon Shedden's 2001 season) were left untouched since there's nothing to regenerate from - `career_stats.py`'s own known gap, see its module docstring.
