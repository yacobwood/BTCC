...
A driver card's `carImageUrl` badge (added 2026-08-21) is the driver's own resolved car - not a shared team image - because a team can field more than one livery: Steel Seal with Power Maxed Racing's Dexter Patterson and Nick Halstead each have their own separately-branded car (Halstead's mid-season "Ask GVT" livery, confirmed by an official photo, differs from Patterson's). Team cards dropped the car cutout entirely for the same reason - a single "representative" car on a team tile would just be misleading once more than one livery exists under that team - and instead show the sponsor logo (`logoUrl`) large and centered, filling most of the tile rather than a small top-right badge. Teams with no logo file yet (e.g. CPRL as of 2026-08-20) simply render without one - no placeholder. `MerchScreen.js`'s tiles use the identical large/centered `teamLogoImgLarge` treatment for parity (see below).
