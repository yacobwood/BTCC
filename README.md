...
The header (added 2026-08-21) also shows the driver's own car - same underlying reason as DriversScreen's tile used to have - a team can field more than one livery, so it has to be the driver's own `carImageUrl`, not a shared team image. Now that the tile has dropped its car entirely (see above), this header is the *only* place it appears. Current layout (by request, after several earlier arrangements didn't land): the car runs the **full header height down the left edge** (`headerCarSide`/`headerCarSideImg`, `CAR_SIDE_TOP` just a small top margin rather than starting partway down, `CAR_SIDE_H` filling the rest via the same `HEADER_W - CAR_SIDE_TOP - CAR_SIDE_MARGIN` formula as before), rotated `90deg` (clockwise) - a 180-degree flip from the previous `-90deg` (anti-clockwise) side-strip, still landing as a vertical column either way since a half-turn on top of a quarter-turn is just the other quarter-turn. `headerNumberImg`/`headerNumber` stay top-right, unmoved throughout every layout this header has tried. The driver photo moved from left-aligned to centered - specifically centered within `headerPhotoWrap`, the region to the *right* of the car column (`left: PHOTO_REGION_LEFT`, i.e. `CAR_SIDE_W + CAR_SIDE_MARGIN*2`), not centered across the whole header - a photo wide enough to look right centered on the full width would overlap the car column, so "driver in the middle" means the middle of what's left after the car takes its column, not the header's literal midpoint. This also meant dropping `headerBg`'s `alignItems`/`justifyContent` entirely, since every direct child (car column, photo wrapper, number) is now absolutely positioned rather than relying on flex flow for the photo's placement. Went through several earlier passes first: a bottom-left overlay, then an upright bottom-right corner badge, then a `-90deg` rotated strip on the right under the number (widened/tightened a few times) - each abandoned in favour of the next once it stopped working at the size the car needed to be. Uses the same pre-generated `-thumb` URL (`carThumbUrl()`) as `TeamDetailScreen`'s per-driver cards - this screen never actually hit the decode-memory cap that originally motivated the thumbnail (only one driver's car is ever on screen here), but there's no reason to decode 15x more bitmap than a column this size needs just because the count happens to be small in this context.
