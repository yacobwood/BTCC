...
The Drivers grid is a plain `ScrollView` + `.map()`, not a virtualized list - every tile mounts (and fires its network images) at once regardless of scroll position, so list position is really request-queue position once concurrent image requests outnumber the connections the device's HTTP client will open at once. The car badge added a 3rd network image per tile (on top of the pre-existing `cardBgUrl`/`numberImageUrl`), which on-device showed up as cars towards the bottom of the list losing that race and exhausting `CachedImage`'s retry budget before the burst cleared. Fixed by staggering just the car badge's mount (`CAR_BADGE_STAGGER_MS` = 150ms per list position, capped at 3s) - the pre-existing two images per tile are untouched, so a tile's name/number/photo still load exactly as fast as before; only the newly-added third request is spread out so it doesn't compound the load. Skipped entirely (no timer at all) for a driver with no `carImageUrl` on record. An initial, smaller stagger (60ms/1.2s) wasn't enough margin - confirmed live, since a car badge is always queued behind the *entire* photo/background burst (~46 requests, declared earlier in every tile's JSX) rather than just behind badges ahead of it, so the tail needs enough delay to actually outlast that whole backlog. `CachedImage`'s own retry budget (`MAX_RETRIES`) was widened 2 -> 5 for the same reason - a request queued this far back can legitimately need several attempts before it gets a turn, not just one or two.
