 ...
 *
 * Property 1: For any master-detail screen (Results) rendered at a screen width
 ...
 */
class ResultsMasterDetailPropertyTest {
    ...
    fun masterDetailLayoutStructureOnTablet() {
        ...
        runBlocking {
            checkAll(PropTestConfig(iterations = 100), Arb.int(600..1200)) { width ->
                ...
                rule.onNodeWithTag("results_master_pane")
                    .assertIsDisplayed()
                    .assertWidthIsEqualTo(360.dp)

                // Divider exists and has 1dp width
                rule.onNodeWithTag("results_divider")
                    .assertIsDisplayed()
                    .assertWidthIsEqualTo(1.dp)

                // Detail pane exists and fills remaining space
                // Use bounds-based check with tolerance for pixel rounding
                val detailBounds = rule.onNodeWithTag("results_detail_pane")
                    .assertIsDisplayed()
                    .getBoundsInRoot()
                ...
                // Placeholder text visible when no round selected
